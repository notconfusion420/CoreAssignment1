...
# Subnets
# -----------------------------
# Public subnet (reserved if needed later, e.g. bastion)
resource "aws_subnet" "public_a" {
  vpc_id                  = aws_vpc.this.id
  cidr_block              = cidrsubnet(var.vpc_cidr, 8, 0) # 10.2.0.0/24
...
resource "aws_subnet" "db_a" {
  vpc_id            = aws_vpc.this.id
  cidr_block        = cidrsubnet(var.vpc_cidr, 8, 1) # 10.2.1.0/24
  availability_zone = var.az_a
...
resource "aws_subnet" "db_b" {
  vpc_id            = aws_vpc.this.id
  cidr_block        = cidrsubnet(var.vpc_cidr, 8, 2) # 10.2.2.0/24
  availability_zone = var.az_b
  tags              = merge(local.tags, { Tier = "private-db", AZ = var.az_b })
}

# -----------------------------
# IGW (optional, not used directly by DB)
# -----------------------------
